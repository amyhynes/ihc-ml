...
def process(subFolder, outputDirectory, filename):
	...
	for row in info:
		if row['Animal_ID'] == filename.replace('s', '-').replace('G', '-').split('-')[0]:		#switch the letter with the first letter of marker after animal id in filename
	...
	for chan in channels:
		...
		fieldnames.append(v+"-cells/tissue-area")
